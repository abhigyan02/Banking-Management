...
import maskpass
import base64
def aadhar(fname, cur):
    while True:
        aadhar=input("Enter aadhar number (integer of length 8 only accepted): ")
        if len(aadhar)==8:
            try:
                aadhar=int(aadhar)
            except ValueError:
                print("Aadhar should be an integer!!")
            else:
                cur.execute("select * from aadhar_verify where aadhar = {}".format(aadhar))
                record=cur.fetchall()
                if record==[]:
                    print("Aadhar number not valid!!")
                    return
                elif record[0][1].lower()!=fname.lower():
                    print("Aadhar number and name do not match!!")
                    return
                print("Aadhar added successfully")
                return aadhar
        else:
            print("Aadhar number should be 8 digits long!!")
...
def clientpassword():
    while True:
            password=maskpass.askpass(prompt="Enter client login password(max 8 characters, min 4, integer only): ", mask="*")
